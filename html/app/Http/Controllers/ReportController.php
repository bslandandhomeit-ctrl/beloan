...
class ReportController extends Controller
{
    ...
    {
        $offset = ($_GET['offset'] != '') ? $_GET['offset'] : 1000;
        ...
        ->whereBetween('loan_payments.repayment_date', [$start, $end])
...
//        }])->where('tran_id', 0);
    ...
    public function getLoanMasterList()
    {
        ...
        $loan = $B0->selectRaw('
                ...
                tb_clients.phone2,
                tb_clients.address,  
                TIMESTAMPDIFF(MONTH,tb_loans.contract_date,tb_loans.contract_deadline)  as handover_dateline,             
                tb_clients.client_type,
                tb_projects.short_code,
                tb_unit_types.name,
                tb_units.code,
                tb_unit_types.extended_deadline,
            ...
            'schedule' => function ($query) {
                $query->select('*')->orderBy('no','asc');
            },
            'feecharge' => function ($query) use ($data) {
                $query->select('loan_id', 'charge_type', 'charge_amount','note');
            },     
        ...
        ->leftJoin('loan_write_off','loan_write_off.loan_id','=','loans.id')
        ->leftJoin('loan_approval', 'loans.id', '=', 'loan_approval.loan_id')
        ->groupBy('loans.id');

           
        // ->whereDate('loans.created_at','>=',$from_date)->whereDate('loans.created_at','<=',$to_date);
        ...
        if (Request::has('customer_id')) {
            ...
            $loan = $loan->where('loans.client_id',$customer_id);
        }
        $company_branch=null;
        ...
        if (Request::has('status')) {
            ...
            switch ($status) {
                case 3: 
                    if($from_date && $to_date){
                        $loan = $loan->whereDate('loans.disburse_date','>=',$from_date)->whereDate('loans.disburse_date','<=',$to_date);
                    }
                    
                    break;
                case 5: 
                    if($from_date && $to_date){
                    $loan = $loan->whereDate('loan_write_off.write_off_date','>=',$from_date)->whereDate('loan_write_off.write_off_date','<=',$to_date);
                    }
                    break;
                case 10:  
                    if($from_date && $to_date){                  
                    $loan = $loan->whereDate('loans.settlement_date','>=',$from_date)->whereDate('loans.settlement_date','<=',$to_date);
                    }
                    break;
                case 11: 
                    if($from_date && $to_date){
            ...
            if($from_date && $to_date){
            $loan = $loan->whereDate('loans.created_at','>=',$from_date)->whereDate('loans.created_at','<=',$to_date);
            }
        }
        $project_id = null;
        ...
        $loan = $loan->paginate($offset)->setPath('loan_collection_detail?client_id='.$customer_id.'&contract_id='.$contract_id.'&status='.$status.'&offset='.$offset);//take($offset)->get();
        // dd($loan);
        ...
        if(Request::input('is_excel') == 1 || Request::input('is_csv') == 1){
            ...
            return Excel::create('loans-'.date('d-M-Y'), function($excel) use ($loan,$product_type) {
                ...
                {
                    $sheet->loadView('exports.loans.loan_master_loan_list',['loans' => $loan,'product_type'=>$product_type]);
                });
            })->download($xlsx);
        }
        // dd($loan);
        return $this->view('reports.loan_master_loan_list', [
            'loans' => $loan, 'status' => $status,'contract_id' => $contract_id,'customer_id'=>$customer_id,
            'client'=>$client,'company_branch'=>$company_branch,
            'project_id' => $project_id,'project' => $project, 
            'unit_type_id' => $unit_type_id,'unit_type' => $unit_type,
            'unit_id' => $unit_id,'unit' => $unit,'from_date'=>$from_date,'to_date'=>$to_date,
            'offset' => $offset, 'product_type' => $product_type]);
    }
    public function getLoanDetailList()
    {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
        $from_date = Request::input('t_from')?Request::input('t_from'):null;
        $to_date = Request::input('t_to')?Request::input('t_to'):null;
        ...
        $loan = $B0->selectRaw('
                ...
                tb_unit_types.name,
                tb_units.code,
                tb_units.building_size_width,
                tb_units.building_size_length,
                tb_units.land_size_width,
                tb_units.land_size_length,
                tb_unit_types.extended_deadline,
                tb_loans.settlement_date,
                tb_loans.rate_type,
                tb_currency.code AS currency_code,
        ...
        if(Request::input('is_excel') == 1 || Request::input('is_csv') == 1){
            ...
            return Excel::create('loans-'.date('d-M-Y'), function($excel) use ($loan,$product_type) {
                $excel->sheet('mySheet', function($sheet) use ($loan,$product_type)
                {
                    $sheet->loadView('exports.loans.loan_detail_list',['loans' => $loan,'product_type'=>$product_type]);
                });
            })->download($xlsx);
        }
        // dd($loan);
        return $this->view('reports.loan_detail_list', [
            'loans' => $loan, 'status' => $status,'contract_id' => $contract_id,'customer_id'=>$customer_id,
            'client'=>$client,'company_branch'=>$company_branch,
            'project_id' => $project_id,'project' => $project, 
            'unit_type_id' => $unit_type_id,'unit_type' => $unit_type,
    ...
    public function getSaleReportDetail()
    {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
		...
		->orderBy('loans.disburse_date', 'asc');
        ...
        if(!empty(Auth::user()->sale_person)){
            ...
            if($sale_person_list->lavel==2){
                ...
                if($sale_person_memberl2){
                    ...
                    $loan = $loan->orWhereIn('sale_order.sale_person',$idsAll); 
        ...
        if(Request::input('is_excel') == 1 || Request::input('is_csv') == 1){
            $listSaleExport=$loan->get();
            $xlsx = 'xlsx';
            if(Request::has('is_csv') == 1){
                $xlsx = 'csv';
            }
            return Excel::create('sale-order-'.date('d-M-Y'), function($excel) use ($listSaleExport) {
                $excel->sheet('mySheet', function($sheet) use ($listSaleExport)
                {
                    $sheet->loadView('exports.sale.list_sale_order',['lists' => $listSaleExport]);
                });
            })->download($xlsx);
        }
 
        $listSale = $loan->paginate($offset)->setPath('sale_report_detail?client_id='.$customer_id.'&company='.$company.'&project_id='.$project_id.'&status='.$status.'&search='.$search.'&offset='.$offset);
//   dd($listSale);
        $contract_template = config('static_data.contract_template');
        return $this->view('sale.list_sale_order', ['lists' => $listSale, 'customer_id' => $company,'company' => $customer_id,'offset'=>$offset,'contract_template' => $contract_template],compact('projects','project_id','customer_id','client','from_date','to_date','company','status'));
    }
    public function listSaleCommission()
    {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
        $from_date = Request::input('t_from')?Request::input('t_from'):null;
        $to_date = Request::input('t_to')?Request::input('t_to'):null;
        ...
        $listSale = $loan->paginate($offset)->setPath('list_sale_commission?company='.$company.'&project_id='.$project_id.'&unit_type_id='.$unit_type_id.'&sale_person='.$sale_person.'&commission_status='.$commission_status.'&search='.$search.'&offset='.$offset);
//   dd($listSale);
        $contract_template = config('static_data.contract_template');
        return $this->view('sale.list_sale_commission', ['lists' => $listSale,'company' => $company,'offset'=>$offset,'contract_template' => $contract_template],compact('projects','project_id','unit_type_id','client','from_date','to_date','company','commission_status','sale_person_list','saleRepresentative','sale_person'));
    }
    public function listPrevCommission()
    {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
        $from_date = Request::input('t_from')?Request::input('t_from'):null;
        $to_date = Request::input('t_to')?Request::input('t_to'):null;
        ...
        $loan = $B0->selectRaw('
            ...
            'coa_journal_detail' => function ($query) {
                ...
                // if(Request::has('t_from')){                   
                //     $query = $query->whereDate('journal_requiry.entry_date','>=', Request::input('t_from'));
                // }
                // if(Request::has('t_to')){                   
                //     $query = $query->whereDate('journal_requiry.entry_date','<=', Request::input('t_to'));
                // }
            },         
            'sale_persons',
            'sale_person_parent_lavel1',
        ...
        ->where('client_loan_accounts.sale_id',null)        
        ->where('loans.commission_status',$commission_status)
		->orderBy('loans.disburse_date', 'desc');
        ...
        if(Request::has('sale_person')){
            $sale_person = Request::input('sale_person');
            $sale_person_list = SalePerson::find($sale_person);
            $loan = $loan->where('loans.sale_person',$sale_person);
            if($sale_person_list->lavel==2){
                $sale_person_memberl2 = SalePerson::where('active',1)->where('parent_id',$sale_person_list->id)->get();
                if($sale_person_memberl2){
                    $ids = [];
                ...
                if($sale_person_memberl2){
                    ...
                    $loan = $loan->orWhereIn('loans.sale_person',$idsAll); 
        ...
        $listSale = $loan->paginate($offset)->setPath('list_prev_commission?company='.$company.'&project_id='.$project_id.'&unit_type_id='.$unit_type_id.'&sale_person='.$sale_person.'&commission_status='.$commission_status.'&search='.$search.'&offset='.$offset);
//   dd($listSale);
        $contract_template = config('static_data.contract_template');
        return $this->view('sale.list_prev_commission', ['lists' => $listSale,'company' => $company,'offset'=>$offset,'contract_template' => $contract_template],compact('projects','project_id','unit_type_id','client','from_date','to_date','company','commission_status','sale_person_list','saleRepresentative','sale_person'));
    }
    public function getSaleReportDetail_bk() {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
        if(!$offset){
        	$offset = 15;
        }
        ...
        ->where('sale_order.invoice_status','Invoice')
        ->where('sale_order.sale_status','New_Sale')
        ->orWhere('sale_order.status','Canceled')
        ...
        $listSale = $listSale->paginate($offset)->setPath('sale_report_detail?client_id='.$customer_id.'&company='.$company.'&project_id='.$project_id.'&status='.$status.'&search='.$search.'&offset='.$offset);
        $contract_template = config('static_data.contract_template');
        return $this->view('sale.list_sale_order', ['lists' => $listSale, 'customer_id' => $company,'company' => $customer_id,'offset'=>$offset,'contract_template' => $contract_template],compact('projects','project_id','customer_id','client','from_date','to_date','company','status'));

    }
    public function getLoanCollectionDtail()
    {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
        $from_date = Request::input('t_from')?Request::input('t_from'):date("Y-m-d");
        $to_date = Request::input('t_to')?Request::input('t_to'):date("Y-m-d");
        ...
        if(Request::input('is_excel') == 1 || Request::input('is_csv') == 1){
            ...
            return Excel::create('loans-'.date('d-M-Y'), function($excel) use ($loan,$product_type) {
                $excel->sheet('mySheet', function($sheet) use ($loan,$product_type)
                {
                    $sheet->loadView('exports.loans.loan_collection_detail',['loans' => $loan,'product_type'=>$product_type]);
                });
            })->download($xlsx);
        }
        return $this->view('reports.loan_collection_detail', [
            'loans' => $loan, 'status' => $status,'contract_id' => $contract_id,'customer_id'=>$customer_id,
            'client'=>$client,'company_branch'=>$company_branch,
            'project_id' => $project_id,'project' => $project, 
            'unit_type_id' => $unit_type_id,'unit_type' => $unit_type,
    ...
    public function getLoanChangeUnite()
    {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
        $contract_date = Request::input('contract_date')?Request::input('contract_date'):date("Y-m-d");
        ...
        ->leftJoin('unit_types','unit_types.id','=','loans.unit_type_id')
        ->leftJoin('units','units.id','=','loans.unit_id')
        ->whereDate('loans.contract_date','>=',$contract_date)->whereDate('loans.contract_date','<=',$date);
        $loan = $loan->where('workflow_status', 'approve');
        $loan = $loan->where('loans.clearance_amount', '>',0);
        ...
        $loan = $loan->paginate($offset)->setPath('loan_change_unit?client_id='.$customer_id.'&contract_id='.$contract_id.'&status='.$status.'&offset='.$offset);//take($offset)->get();
        // dd($loan);
        ...
        return $this->view('reports.loan_change_unit', [
            'loans' => $loan, 'status' => $status,'contract_id' => $contract_id,'customer_id'=>$customer_id,
            'client'=>$client,'company_branch'=>$company_branch,
            'project_id' => $project_id,'project' => $project, 
            'unit_type_id' => $unit_type_id,'unit_type' => $unit_type,
    ...
    public function getLoanTransferClients()
    {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
        $from_date = Request::input('t_from')?Request::input('t_from'):date("Y-m-d");
        $to_date = Request::input('t_to')?Request::input('t_to'):date("Y-m-d");
        $query_arr = array('user_id' => Auth::user()->id, 'company_branch_id' => Auth::user()->branch_id, 'role_id' => Auth::user()->role_id);
        $B0 = new TransferClient();
        $loan = $B0->selectRaw('
                tb_loans.id,
                tb_loans.contract_id,
                tb_loans.start_date,
                ...
                tb_loans.rate_type,
                tb_company_branch.short_name as company,
                tb_transfer_clients.created_at as transfer_date,
                tb_transfer_clients.transfer_type,
                tb_transfer_clients.Remark
        ...
        ->Join('loans', 'loans.id', '=', 'transfer_clients.loan_id')
        ->leftJoin('projects','projects.id','=','loans.project_id')
        ->leftJoin('company_branch','company_branch.id','=','projects.company_id')
        ->leftJoin('unit_types','unit_types.id','=','loans.unit_type_id')
        ->leftJoin('units','units.id','=','loans.unit_id')
        ->whereDate('transfer_clients.created_at','>=',$from_date)->whereDate('transfer_clients.created_at','<=',$to_date);
        ...
        if (Request::has('transfer_type')) {
            ...
            $loan = $loan->where('transfer_clients.transfer_type', '=', $transfer_type);
            $query_url['transfer_type'] = $transfer_type;
        }
        $project_id = null;
        if(Request::has('project_id')){
            $project_id = Request::input('project_id');
        ...
        $loan = $loan->paginate($offset)->setPath('loan_transfer_clients?client_id='.$customer_id.'&contract_id='.$contract_id.'&transfer_clients='.$transfer_clients.'&offset='.$offset);//take($offset)->get();
        ...
        if(Request::input('is_excel') == 1 || Request::input('is_csv') == 1){
            ...
            return Excel::create('loan_transfer_clients_'.date('d-M-Y'), function($excel) use ($loan) {
                $excel->sheet('mySheet', function($sheet) use ($loan)
                {
                    $sheet->loadView('exports.loans.loan_transfer_client',['loans' => $loan]);
                });
            })->download($xlsx);
        }
        return $this->view('reports.loan_transfer_clients', [
            'loans' => $loan, 'transfer_clients' => $transfer_clients,'contract_id' => $contract_id,'customer_id'=>$customer_id,
            'client'=>$client,'company_branch'=>$company_branch,
            'project_id' => $project_id,'project' => $project, 
            'unit_type_id' => $unit_type_id,'unit_type' => $unit_type,
            'unit_id' => $unit_id,'unit' => $unit,'from_date'=>$from_date,'to_date'=>$to_date,
    ...
    public function getLoanRestructureList()
    {
        $offset = isset($_GET['offset']) ? $_GET['offset'] : 15;
        $from_date = Request::input('t_from')?Request::input('t_from'):date("Y-m-d");
        $to_date = Request::input('t_to')?Request::input('t_to'):date("Y-m-d");
        $query_arr = array('user_id' => Auth::user()->id, 'company_branch_id' => Auth::user()->branch_id, 'role_id' => Auth::user()->role_id);
        $B0 = new LoanRestructure();
        $loan = $B0->selectRaw('
                tb_loans.id,
                tb_loans.contract_id,
                tb_loans.start_date,
        ...
        ->Join('loans', 'loans.id', '=', 'loan_restructure.loan_id')
        ->leftJoin('clients', 'clients.id', '=', 'loans.client_id')
        ->leftJoin('projects','projects.id','=','loans.project_id')
        ->leftJoin('company_branch','company_branch.id','=','projects.company_id')
        ->leftJoin('unit_types','unit_types.id','=','loans.unit_type_id')
        ->leftJoin('units','units.id','=','loans.unit_id')
        ->whereDate('loan_restructure.restructure_date','>=',$from_date)->whereDate('loan_restructure.restructure_date','<=',$to_date);
        ...
        if (Request::has('transfer_type')) {
            $transfer_type = Request::input('transfer_type');
            $loan = $loan->where('loan_restructure.transfer_type', '=', $transfer_type);
            $query_url['transfer_type'] = $transfer_type;
        }
        $project_id = null;
        if(Request::has('project_id')){
        ...
        $loan = $loan->paginate($offset)->setPath('loan_restructure?client_id='.$customer_id.'&contract_id='.$contract_id.'&transfer_clients='.$transfer_clients.'&offset='.$offset);//take($offset)->get();
        ...
        if(Request::input('is_excel') == 1 || Request::input('is_csv') == 1){
            ...
            return Excel::create('loan_restructure_'.date('d-M-Y'), function($excel) use ($loan) {
                $excel->sheet('mySheet', function($sheet) use ($loan)
                {
                    $sheet->loadView('exports.loans.loan_restructure',['loans' => $loan]);
                });
            })->download($xlsx);
        }
        return $this->view('reports.loan_restructure', [
            'loans' => $loan, 'transfer_clients' => $transfer_clients,'contract_id' => $contract_id,'customer_id'=>$customer_id,
            'client'=>$client,'company_branch'=>$company_branch,
            'project_id' => $project_id,'project' => $project, 
            'unit_type_id' => $unit_type_id,'unit_type' => $unit_type,
