function loadModale(obj) {
    ...
    require(obj.script);
    var vals_date = $('input[name=vals_date]').val();
    var status = 0;
    if ($('input[name=status]').val() == 0) {
        status = 1;
    }
    submitpost("." + obj.idSelector, obj.byId, status, vals_date);
}
...
function submitpost(selector, byId, status, vals_date) {
    ...
    $(selector).validate({
        ...
        submitHandler: function () {
            ...
            $.ajax({
                ...
                beforeSend: function () {
                    imgLoading(true, "Checking...", 0.2, 'no');
                }, statusCode: {
                    500: function () {
                        if (confirm('Sorry!!!, You can try again if not so please click Ok to reload page')){
                ...
                success: function (data, status) {
                    ...
                    if (status == 'success') {
                        ...
                        if (data.permis == false) {
                            ...
                            imgLoading(true, "Permission Fail", 10, status);
                            $("#result").html('');
                        }
                        if (data.res === false) {
                            imgLoading(true, "Fail", 10, status);
                            $("#result").html('');
                        } else {
