...
function findStatusBykey($status,$array){
    ...
    $status=null;
    $status_btn=null;
        if(!empty($unit_type->activeSaleCommissionSetting) && count($unit_type->activeSaleCommissionSetting) > 0){
        $status=$unit_type->activeSaleCommissionSetting->approval;
        if($status==='approved'){
                $status='Approved';               
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="commission_type">
                                        <option value="">-</option>
                                        <option value="$" {{ @$unit_type->activeSaleCommissionSetting->commission_type == '$' ? 'selected' : '' }}>Fixed ($)</option>
                                        <option value="%" {{ @$unit_type->activeSaleCommissionSetting->commission_type == '%' ? 'selected' : '' }}>Percentage (%)</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="form-group">
                                    <label class="control-label" style="width:100%;text-align: left;">Status</label>
                                    <p class="{{$status_btn}}" >{{$status}}</p>
                                
                                </div>
                            </div>
                            <div class="form-group">
                            <label class="col-sm-12 control-label">{{ trans('multiple.m_note') }}</label>
                            <div class="col-sm-12">
                                <textarea class="form-control" readonly  id="note" name="note" value="{{ @$unit_type->activeSaleCommissionSetting->note }}">{{ @$unit_type->activeSaleCommissionSetting->note }}</textarea>
                            </div>
                        </div>
                        </div>
                    </div>
