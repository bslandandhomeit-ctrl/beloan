...
@section('content')
    <section class="panel">
        <header class="panel-heading">
            {{ trans('sidebar.sb_fa_detail') }}
        </header>

        <div class="panel-body">
            <div class="position-center text-center">
                <form role="form" class="cmxform form-inline" method="get" action="{{ route('fa_detail') }}">
            ...
            <section id="unseen">
                ...
                <div id="printArea">
                    ...
                    <div id="ex-print">
                        <p class="table-smaller">
                            <span>@if($as_at) As at {{$as_at}} @endif</span>
                            <span class="pull-right">@if($last_record) Prior Date: {{date('Y-m-d', strtotime($last_record))}} @endif</span>
                        </p>
                    </div>
                    <table class="table table-bordered table-striped table-condensed table-smaller">
                        <thead>
                            <tr>
                                ...
                                <th rowspan="2">{{ trans('asset.supplier') }}</th>
                                <th rowspan="2">{{ trans('asset.invoice_num') }}</th>
                                <th colspan="4" class="text-center">{{ trans('asset.value_of_assets') }}</th>
                                <th colspan="6" class="text-center">{{ trans('asset.accumulate_deprecation') }}</th>
                                <th colspan="2" class="text-center">{{ trans('asset.net_book') }}</th>
                            </tr>

                            <tr>
                                <th>{{ trans('asset.existing_assets') }}</th>
                                <th>{{ trans('asset.new_asset_adding') }}</th>
                                <th>{{ trans('asset.disposed') }}</th>
                                ...
                                <th>{{ trans('asset.writen_back') }}</th>
                                <th>{{ trans('asset.total_depre') }}</th>
                                <th>{{ trans('asset.prio_value') }}</th>
                        ...
                        <tbody>
                            ...
                            <?php
                                $total_existing_asset = 0;
                                $total_new_asset_adding = 0;
                                $total_dispose = 0;
                                $total_total_cost = 0;
                                $total_dpr = 0;
                                $total_dprn = 0;
                            ...
                            <?php
                                $total_existing_asset += $t_existing_asset;
                                $total_new_asset_adding += $t_new_asset_adding;
                                $total_dispose += $t_dispose;
                                $total_total_cost += $t_total_cost;
                                $total_dpr += $t_dpr;
                                $total_dprn += $t_dprn;
                                $total_writen_back += $t_writen_back;
                                $total_total_dpr += $t_total_dpr;
                                $total_prio_net_book += $t_prio_net_book;
                                $total_net_book_value += $t_net_book_value;
                            ...
                            <!--*****************************-->
                            <tr><td colspan="20">{{ trans('asset.leasehold_improvement') }}</td></tr>
                            <?php
                                $total_existing_asset += $t_existing_asset;
                                $total_new_asset_adding += $t_new_asset_adding;
                                $total_dispose += $t_dispose;
                            ...
                            <!--*****************************-->
                            <tr><td colspan="20">{{ trans('asset.furniture_fixture') }}</td></tr>
                            <?php
                                $total_existing_asset += $t_existing_asset;
                                $total_new_asset_adding += $t_new_asset_adding;
                                $total_dispose += $t_dispose;
                            ...
                            <!--*****************************-->
                            <tr><td colspan="20">{{ trans('asset.equipment') }}</td></tr>
                            <?php
                                $total_existing_asset += $t_existing_asset;
                                $total_new_asset_adding += $t_new_asset_adding;
                                $total_dispose += $t_dispose;
                            ...
                            <!--*****************************-->
                            <tr><td colspan="20">{{ trans('asset.computer_equipment') }}</td></tr>
                            <?php
                                $total_existing_asset += $t_existing_asset;
                                $total_new_asset_adding += $t_new_asset_adding;
                                $total_dispose += $t_dispose;
                            ...
                            <!--*****************************-->
                            <tr><td colspan="20">{{ trans('asset.motor_vehicles') }}</td></tr>
                            <?php
                                $total_existing_asset += $t_existing_asset;
                                $total_new_asset_adding += $t_new_asset_adding;
                                $total_dispose += $t_dispose;
                            ...
                            <!--*****************************-->
                            <tr><td colspan="20">{{ trans('asset.computer_software') }}</td></tr>
                            <?php
                                $total_existing_asset += $t_existing_asset;
                                $total_new_asset_adding += $t_new_asset_adding;
                                $total_dispose += $t_dispose;
                            ...
                            <!--*****************************-->
                            <tr><td colspan="20">{{ trans('asset.formation_epense') }}</td></tr>
                            <?php
                                $total_existing_asset += $t_existing_asset;
                                $total_new_asset_adding += $t_new_asset_adding;
                                $total_dispose += $t_dispose;
                            ...
                            <!--GRAND TOTAL-->
                            <tr class="acc1">
                                <td colspan="7" class="text-right">{{ trans('multile.grand_total') }}</td>
                                <td>{{number_format($total_existing_asset)}}</td>
                                <td>{{number_format($total_new_asset_adding)}}</td>
                                <td>{{number_format($total_dispose)}}</td>
                                <td>{{number_format($total_total_cost)}}</td>
                                <td></td>
                                <td></td>
                                <td>{{number_format($total_dpr)}}</td>
                                <td>{{number_format($total_dprn)}}</td>
                                <td>{{number_format($total_writen_back)}}</td>
                                <td>{{number_format($total_total_dpr)}}</td>
                                <td>{{number_format($total_prio_net_book)}}</td>
                                <td>{{number_format($total_net_book_value)}}</td>
                            </tr>
