...
@section('content')
    <div class="row">
        <form class="form-horizontal" method="post" action="{{route('edit_sale_person',[$sale_person->id])}}">
            <div class="col-sm-12">
                <section class="panel">
                    <header class="panel-heading">
                        {{ trans('sale_person.edit_sale_person') }}
                    </header>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="sale_team_id" required>
                                        ...
                                        @foreach ($co_name as $val)
                                            <option value="{{ $val->id }}" {{ $val->id?$val->id == $sale_person->sale_team_id?"selected":"":old('sale_person') }}>{{ $val->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" value="{{ $sale_person->name }}" class="form-control" required/>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="gender" required>
                                        <option value="male" {{ $sale_person->gender?$sale_person->gender?"selected":"":old('sale_person') }}>Male</option>
                                        <option value="female" {{ $sale_person->gender?$sale_person->gender?"selected":"":old('sale_person') }}>Female</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="dob" data-date-viewmode="years" data-initialize="datepicker" data-date-format="dd/mm/yyyy" class="form-control input-append date dpYears" size="16" value="{{ isset($sale_person->dob)?$sale_person->dob:date('Y-m-d') }}" required/>
                        ...
                        <div class="row">
                                    ...
                                    <select class="form-control" name="national_id" required>
                                        ...
                                        @foreach ($nationlity['countries'] as $val)
                                            @foreach ($val->description as $row)
                                                <option value="{{ $row->country_id }}" {{ $row->country_id?$row->country_id == $sale_person->national_id?"selected":"":old('sale_person') }}>{{ $row->name }}</option>
                                            @endforeach
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="email" name="email" value="{{ $sale_person->email }}" class="form-control" required/>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="form-group">
                                    <label class="control-label">{{ trans('sale_person.phone') }} <span class="red-color">*</span></label>
                                    <input type="text" name="phone" class="form-control" value="{{ $sale_person->phone }}" id="phone" data-mask="999-999-999?9" placeholder="___-___-____"  required/>
                                </div>
                            </div>
                        </div>
                        <div class="row">
