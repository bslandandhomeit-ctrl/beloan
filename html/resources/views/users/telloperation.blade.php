    ...
    <script type="text/javascript">
        ...
        var status_flag = '<?php echo $flag;?>';
            if(status_flag == 1){ // all account opened
                ...
                $(".transferTill").setAttribute("disabled", "disabled");
                $(".repayLoan").setAttribute("disabled", "disabled");
                $(".incomeFee").setAttribute("disabled", "disabled");
                $(".DisburseLoan").setAttribute("disabled", "disabled");
...
//            });
            ...
            $(document).on('keyup', '#sdeposit #amount, #swithdraw #amount', function () {

                if($(this).parents('#swithdraw').find('select[name="client"]').length==1){
                    frmm = "name="+$('select[name="client"]').find('option:selected').text()+'&id='+$('select[name="client"]').val();
                    $.ajax({
                        url: "{{ route('teller_dwjournal', 2) }}",
                        data: frmm+'&amount='+$(this).val(),
                        method: 'get',
                        success: function (res) {
                            $('.teller_dwjournal').remove();
                            $('#swithdraw .modal-body .container').append(res);
                        }
                    })

                }else{
                    frmm = "name="+$('select[name="from"]').find('option:selected').text()+'&id='+$('select[name="from"]').val();
                    $.ajax({
                        url: "{{ route('teller_dwjournal', 1) }}",
                        data: frmm+'&amount='+$(this).val(),
                        method: 'get',
                        success: function (res) {
                            $('.teller_dwjournal').remove();
                            $('#deposit .modal-body .container').append(res);
                        }
                    })
                }
            });
