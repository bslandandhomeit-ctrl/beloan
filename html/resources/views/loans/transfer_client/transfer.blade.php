        ...
        <section class="panel">
           @if(Session::has('error'))
            ...
            <header class="panel-heading">
                {{ trans('Transfer Clients') }}
            </header>
            <div class="panel-body" style="margin: 0 auto;">
                <p><span class="text-danger">NOTE: Please Make sure you want transfer Customer !</span></p>
                <form class="cmxform form-horizontal" method="post" action="{{ route('transfer_client_store', [$id]) }}" id="frmloanGuarantor">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label class="col-sm-3 control-label">{{ trans('loan.l_transfer_category') }} <span class="red">*</span></label>
                                    <div class="col-md-8">
                                        <select class="form-control" name="transfer_client_type" id="transfer_client_type" required>
                                            <option value="">-</option>
                                            @foreach($static['transfer_client_type'] as $key => $value)
                                                <option value="{{ $value }}" {{ $value == $co_borrower->relationship ? 'selected' : '' }}>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="control-label col-sm-3">From {{ trans('customer.cus_customer') }} <span class="red">*</span></label>
                                    <div class="col-md-8">
                                        <input type="text" name="customer_id" class="form-control" value="{{ $client_name }}" readonly>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <style type="text/css">
                                    ...
                                    <label class="control-label col-sm-3">To {{ trans('customer.cus_customer') }} <span class="red">*</span></label>
                                    {{-- <div id="customer_id"  class="col-md-8"></div> --}}
                                    <div class="col-md-8">
                                        <input type="text" name="customer_id" class="form-control" id="customer_id" required style="width: 300px; height:0px">
                                    </div>
                                </div>
                                 <div class="form-group">
                                    <label class="control-label col-sm-3">{{ trans('customer.remark') }} </label>
                                    <div class="col-md-8">
                                        <textarea name="remark" id="remark" class="form-control" rows="5">{{ $loan->loan_purpose }}</textarea>
                                    </div>
                                </div>
     ...
     <script type="text/javascript">
        ...
        function select() {
            $("#customer_id").select2({
                ...
                ajax: {
                    url: '{{ asset('loan-borrower/get_search_borrower')}}',
                    dataType: 'json',
                    type: "GET",
                    quietMillis: 50,
                    timeout: 3000,
