...
@section('content')
    <section class="panel">
        ...
        <?php
            ...
            $penalty_amount = 0;
            foreach($sch_repay_arr as $key => $ar){
                $penalty_amount += $ar['penalty'];
            }
            $penalty_days = \Request::get('penalty_days');
            $penalty_months = \Request::get('penalty_month');
        ?>
        <div class="panel-body">
            ...
            <input type="hidden" name="downpayment" id="iDownpayment">
            <div class="form-group">
                ...
                <div class="input-append date dpYears col-md-6" data-date-viewmode="years" data-initialize="datepicker" data-date-format="dd/mm/yyyy" data-date="{{date('Y-m-d')}}">
                    ...
                    <input type="text" value="{{ \Request::get('dpDate')?\Request::get('dpDate'):date('Y-m-d') }}" selected class="form-control" name="dpDate" id="dpDate">
            ...
            <div class="form-group">
                <label class="control-label col-sm-3">Waive interest</label>
                <div class="col-md-6">
                    <div class="flat-green-interest single-row icheck group">
                        <div class="radio" >
                            <input type="checkbox" name="ch_waive_interest" value="1" @if($draft->ip_ch_waive_interest==1) checked="checked" @endif />
                        </div>
                    </div>
                    <div class="input_waive_interest ihide">
                        <input type="text" name="waive_interest_amount" class="form-control" id="waive_interest_amount" value="{{$draft->ip_waive_interest_amount}}" />
                    </div>
                </div>
            </div>
            <div class="form-group">
                <label class="control-label col-sm-3">Accrual  interest</label>
                <div class="col-md-6">
                    <div class="flat-green-accrual_interest single-row icheck group">
                        <div class="radio" >
                            <input type="checkbox" name="ch_accrual_interest"  id="ch_accrual_interest" value="1" {{ (1 == Request::get('is_accrual_interest'))?'checked' : "" }} />
                        </div>
                    </div>

              
                <label class="col-md-3 control-label input_accrual_interest">Accrual Date</label>
                <div class="input-append date dpYears col-md-6 input_accrual_interest" data-date-viewmode="years" data-initialize="datepicker" data-date-format="dd/mm/yyyy" data-date="{{date('Y-m-d')}}">
                    <input type="text" value="{{ \Request::get('accrualDate')?\Request::get('accrualDate'):''}}" selected class="form-control" name="accrualDate" id="accrualDate">
                        <span class="add-on offonDatepicker">
                            <button class="btn btn-primary" type="button"><i class="fa fa-calendar"></i></button>
                    </span>
                </div>
                    ...
                    <div class="input_accrual_interest ihide">
                        <input type="text" name="accrual_interest_amount" class="form-control" id="accrual_interest_amount" value="{{number_format($interest_accruled,2,'.','')}}" />
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-12" id="jd-block">
            ...
            @if(isset($loan->client_loan_account->balance_downpayment) && $loan->client_loan_account->balance_downpayment > 0)
            <div class="b-block act_principal">
                <h5><strong>Down Payment Repayment</strong></h5>
                <?php
                $params = array(
                    'debit_down' => ($loan->client_loan_account)?number_format($loan->client_loan_account->balance_downpayment,2, '.', ''):0.00,
                    'credit_down' => ($loan->client_loan_account)?number_format($loan->client_loan_account->balance_downpayment,2, '.', ''):0.00,
                    'parent_debit_down' => $coa_pcp->id,
                    'parent_credit_down' => $coa->id,
                    'parent_debit_label_down' => $coa_pcp->name . ' (' . $branch_code . $coa_pcp->account_code . ')',
                    'parent_credit_down_label' => $coa->name . ' (' . $branch_code . $coa->account_code . ')',
                    'd_description_down'=>($draft->d_description4 != "")?$draft->d_description4 : "",
                    'c_description_down'=>($draft->c_description4 != "")?$draft->c_description4 : "",
...
@section('js')
    ...
    <?php }else{ ?>
        ...
        $('.flat-green-interest input').iCheck({
            checkboxClass: 'icheckbox_flat-green',
            radioClass: 'iradio_flat-green'
        }).on('ifToggled', function (e) {
            e.preventDefault();
            var chck = $(this).prop('checked');
            if (chck) {
                $('.input_waive_interest').show();
            } else {
                $('.input_waive_interest').hide();
                $('.input_waive_interest input').val('');
            }
            calculatePayOff();
            $('#btnPreview').trigger('click');
        });

        $('.flat-green-accrual_interest input').iCheck({
            checkboxClass: 'icheckbox_flat-green',
            radioClass: 'iradio_flat-green'
        }).on('ifToggled', function (e) {
            e.preventDefault();
            var chck = $(this).prop('checked');
            if (chck) {
                $('.input_accrual_interest').show();
            } else {
                $('.input_accrual_interest').hide();
                $('.input_accrual_interest input').val('');
            }
            get_penalties();
    ...
    var down_payment = <?php echo !empty($loan->client_loan_account->balance_downpayment)?$loan->client_loan_account->balance_downpayment:0;?>;
    ...
    var loan_arr = <?php echo json_encode($loan);?>;
    var sch_repay_arr = <?php echo json_encode($sch_repay_arr);?>;
    var HOLIDAY_FLAG = 0;
    var foo = false;
    var penalty_list = [];
    ...
    $('#button').click(function(){
        ...
        $('#accrualDate').change(function(event){
            event.preventDefault();
            var accrualDate = $(this).val();
            // prevent datapicker firing 3 times
            var send = true;
            if (typeof(event) == 'object'){
                if (event.timeStamp - lastJQueryTS < 300){
        ...
        function get_penalties(){
            ...
            var accrual_interest = $('.flat-green-accrual_interest input').prop('checked');
            var accrualDate = $('#accrualDate').val();
            let is_accrual_interest=0;
            if(accrual_interest){
                $('.input_accrual_interest').show();
                is_accrual_interest=1;
            }else{
                is_accrual_interest=0;
                $('.input_accrual_interest').hide();
                $('.input_accrual_interest input').val('');
            }
            ...
            var penalty_days = 0;
            var penalty_month = 0;
            var penaltyday = "{{$penalty_days}}";
            var penaltymonth = "{{$penalty_months}}";
            let iterations = payoff.overdue_days.length;
            for(var i = 0; i < payoff.overdue_days.length; i++){
                if(!--iterations){
                    penalty_days = payoff.overdue_days[i];
                }
                penalty_month = payoff.overdue_days.length;
            }
            if(penaltymonth != ""){
                $('#ipTenure').attr('value',penaltymonth);
            }else{
                $('#ipTenure').attr('value',penalty_month);
            }
            if(penalty_days >=0 ){
                if(penaltyday != ""){
                    $('#ipTenureDate').attr('value',penaltyday);
                }else{
                    $('#ipTenureDate').attr('value',penalty_days);
                }
            }
            window.history.pushState("", "", `?dpDate=${dpDate}&penalty_days=${penalty_days}&penalty_month=${penalty_month}&is_accrual_interest=${is_accrual_interest}&accrualDate=${accrualDate}`);
        }

        $(document).ready(function() {
            payoff = calculatePayOff();
            fillFrm(payoff);
            penalty_list = <?php echo json_encode($sch_repay_arr);?>;
            get_penalties();
            $('#btnPreview').trigger("click");
        });
        ...
        function calculatePayOff(){
            ...
            /* Waiv Interest */
            var waive_interest_amount = $('.input_waive_interest input').val();
            if(!waive_interest_amount || waive_interest_amount=='' || waive_interest_amount==null){
                waive_interest_amount = 0;
            }
            waive_interest_amount = parseFloat(waive_interest_amount);
            /* End */
            ...
            $.each(repaymentSchData, function(key, sch){
                ...
                total_principal += parseFloat(sch.principal);
            });
            total_principal = (parseFloat(total_principal) - parseFloat(down_payment));
            if(repaymentSchData[imonth].date_num == 0) repaymentSchData[imonth].date_num = 1;
            total_sch_fee += tenure_date * parseFloat(repaymentSchData[parseFloat(imonth)+1].fee)/ parseFloat(repaymentSchData[parseFloat(imonth)+1].date_num);
            if(isNaN(total_sch_fee)) {
                total_sch_fee  = 0;
            }
            ...
            $.each(accrued_int_arr, function(key, acc){
                ...
                if(parseFloat(acc.debit) > 0){
                    last_acc_date = entry_date;
                } 
            ...
            if(!last_acc_date){
                last_acc_date = loan_arr.disburse_date;
            } 
			...
			*/
            ...
            var subTotal2 = Math.round((parseFloat(total_principal) - actualPaid)*(payoff_rate/100)*100)/100 + round(total_sch_fee - actualFee); //payoff fee
            var subTotal3 = 0.0;
            var principal = parseFloat(total_principal) - parseFloat(actualPaid);
            ...
            if (penalty_list != 'undefined'){
                ...
                if(count > 0){
                    $.each(penalty_list, function(key, v){
                        // console.log(v);
                        if(v.penalty > 1){
                            var penalty_days = dateDiff(v['schedule_date'], end, 1, false);
                            schedule_month_arr.push(v['schedule_date']);
                            overdue_arr.push(penalty_days);
                            total_arrear_arr.push(v['penalty']);
                            penalty_arr.push(round(v['penalty'],2));
                            subTotal3 += v[8];
                        }
                    });
                }
            ...
            var totalPayOff = (parseFloat(total_principal) + parseFloat("{{$interest}}")) - actualPaid + subTotal2 + parseFloat("{{$penalty_amount}}");//subTotal3;
            var data = {
		        ...
		        payoff_fee: (round((parseFloat(total_principal) - actualPaid)*(payoff_rate/100)*100)/100).toFixed(2),
                ...
                total3: (round("{{$penalty_amount}}")).toFixed(2),
                ...
                down_payment: (round(down_payment)).toFixed(2)
            };
            data.total3 = data.total3 - waive_panalty_amount;
            data.interest = data.interest - waive_interest_amount;
            data.totalPayOff = data.totalPayOff - waive_panalty_amount - waive_interest_amount;
            ...
            $('#iDownpayment').val(data.down_payment);
            if(round(data.totalPayOff).toFixed(2) > round(drawdown_acc.balance).toFixed(2)){
        ...
        $('#btnPreview').click(function(){
            if(isNumeric($('#ipInterest').val()) && isNumeric($('#ipTenure').val())){
                ...
                // console.log(payoff)
                var loanbrand = '';
                if(loan_data.down_payment === null ){
                    loan_data.down_payment = 0;
                }
                ...
                detail +='<div class="row">\
                    ...
                    <table class="table table-bordered table-striped table-condensed payoff_repayment">\
                        ...
                        <tbody>\
                            <tr>\
                                <td>{{ trans('company.company_name') }}</td><td>'+loan_data.branch.branch_name+'</td>\
                            </tr>\
                            <tr>\
                                <td>{{ trans('unit_type.unit_type_name') }}</td><td>'+loan_data.unittypes.name+'</td>\
                            </tr>\
                            <tr>\
                                <td>{{ trans('unit.unit_code') }}</td><td>'+loan_data.units.code+'</td>\
                            </tr>\
                            <tr>\
                                <td>{{ trans('product.p_product_price') }}</td><td>'+loan_data.unit_sale_price+'</td>\
                            </tr>\
                            <tr class="hidden">\
                ...
                <div class="row">\
                    ...
                    <table class="table table-bordered table-striped table-condensed payoff_repayment">\
                        ...
                        <tbody>\
                            ...
                            <tr>\
                                <td>{{ trans('report.rpt_tenure') }}</td><td>'+ tenure +'M '+ tenure_date +'D ' + '( '+ {{ $interest_day }} +'days )</td>\
                            </tr>\
                        </tbody>\
                    </table>\
                    </div>\
                ...
                <div class="row">\
                    ...
                    <table class="table table-bordered table-striped table-condensed payoff_repayment">\
                        <thead>\
                            ...
                            <tr><th>{{ trans('multiple.m_month') }}</th><th>{{ trans('report.rpt_overdue') }}</th><th class="hidden">{{ trans('loan.l_unpaid_amount') }}</th><th>{{ trans('report.rpt_penalty') }}</th></tr>\
                        </thead>\
                        <tbody>';
                        //console.log(payoff);
                        for(var i = 0; i < payoff.overdue_days.length; i++){
                            ...
                            //if(payoff.schedule_month[i] != 'undefined'){
                                ...
                                detail +=
                                    '<tr>\
                                        ...
                                        <td class="hidden">$'+payoff.total_arrears[i]+'</td>\
                                        <td>$'+payoff.penalty[i]+'</td>\
                                    </tr>';
                            //}
                        }
                detail +='<tr><th>{{ trans('loan.l_sub_total')  }} (III)</th><th></th><th class="hidden"></th><th>$ '+ payoff.total3 + '</th></tr>\
                        </tbody>\
                    </table>\
                    </div>\
                </div>\
                <h5 style="color: red"><strong>{{ trans('loan.detail') }}</strong></h5>\
                 <div class="row">\
                    ...
                    <table class="table table-bordered table-striped table-condensed payoff_repayment">\
                        ...
                        <tbody>\
                            <tr>\
                                <td>{{ trans('report.rpt_principal')  }}</td><td style="text-align: right;">$ '+ payoff.principal+'</td>\
                            </tr>\
                            <tr>\
                                <td>{{ trans('loan.l_down_payment') }}</td>\
                                <td style="text-align: right;">{{$loan->client_loan_account->currencies->symbol}} '+down_payment.toFixed(2)+'</td>\
                            </tr>\
                            <tr>\
                            ...
                            <tr>\
                                <th>{{ trans('loan.total')  }}</th><th style="color:red; text-align: right;">$ '+ (payoff.totalPayOff + down_payment).toFixed(2) +'<input type="hidden" id="inputTotalPayOff" name="inputTotalPayOff" value="'+ (payoff.totalPayOff + down_payment).toFixed(2) +'" /></th>\
                            </tr>\
                        </tbody>\
                    </table>\
                     </div>\
                 </div>\
                <h4><strong>{{ trans('loan.l_total_repayment') }} : <span style="color:red" >US$ '+ (payoff.totalPayOff + down_payment).toFixed(2) + '</span></strong></h4>';
        ...
        $('.input_waive_interest input').keyup(function(){
            payoff = calculatePayOff();
            fillFrm(payoff);
        });
        ...
        function fillFrm(payoff){
            ...
            $('#iDownpayment').val(payoff.down_payment);
            $('#btnPreview').trigger('click');
        }
    </script>
@endsection
