...
@section('content')
    <section class="panel">
        ...
        <div class="panel-body">
            ...
            @endif
            <form class="cmxform form-horizontal" method="post" action="{{ route('loan_disburse',[$loan->id]) }}" id="frmDisburse">
                ...
                <div class="form-group">
                    ...
                    <div class="input-append col-sm-6">
                        <input type="text" placeholder="Select a date" value="{{ date('Y-m-d') }}" class="form-control" name="disburse_on" id="disburse_on" readonly />
                ...
                <div class="form-group">
                    ...
                    <div class="input-append col-sm-6">
                        <input type="text" placeholder="Select a date" value="{{ date('Y-m-d') }}" class="form-control" name="contract_date" id="contract_date" readonly="readonly"/>
                        <span class="add-on offonDatepicker">
                        <button class="btn btn-primary" type="button"><i class="fa fa-calendar"></i></button>
                    </span>
                    </div>
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label">{{ trans('loan.contract_deadline') }} <span class="red">*</span></label>
                    <div class="input-append date dpYears col-sm-6" data-date-viewmode="years" data-initialize="datepicker" data-date-format="dd/mm/yyyy" data-date="{{date('Y-m-d', strtotime($loan->contract_deadline))}}">
                        <input type="text" placeholder="Select a date" value="{{ date('Y-m-d', strtotime($loan->contract_deadline))  }}" class="form-control" name="contract_deadline" id="contract_deadline"/>
                        <span class="add-on offonDatepicker">
                            <button class="btn btn-primary" type="button"><i class="fa fa-calendar"></i></button>
                        </span>
                    </div>
                ...
                <div class="form-group hidden">
                    ...
                    <div class="input-append date dpYears col-sm-6" data-date-viewmode="years" data-initialize="datepicker" data-date-format="dd/mm/yyyy" data-date="{{date('Y-m-d', strtotime($loan->schedule[1]->schedule_date))}}">
                        <input type="text" placeholder="Select a date" value="{{ date('Y-m-d', strtotime($loan->schedule[1]->schedule_date))  }}" class="form-control" name="schedule_date" id="start_date"/>
                        <span class="add-on offonDatepicker">
                            <button class="btn btn-primary" type="button"><i class="fa fa-calendar"></i></button>
                        </span>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        <input type="text" readonly value="{{ $currency_arr[$currency] }}" class="form-control" name="currency_"/>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-3 control-label">{{ trans('loan.l_transaction_amount') }} <span class="red">*</span></label>
                    <div class="col-sm-6">
                        <input type="text" readonly placeholder="Enter transaction amount" value="{{ $loan->loan_amount }}" class="form-control" name="amount" id="amount"/>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label">{{ trans('loan.l_currency') }}</label>
                    <div class="col-sm-6">
                        <input type="text" readonly value="{{$currency_arr[$currency]}}" class="form-control" name="currency_name"/>
                        <input type="hidden" value="{{$currency}}" class="form-control" name="currency" id="currency"/>
                        <input type="hidden" value="{{$loan->client_loan_account->currencies['symbol']}}" class="form-control" name="currency_symbol" id="currency_symbol"/>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        <input type="text" readonly value="{{$loan->contract_id}}" class="form-control" name="contract_id"/>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-3 control-label">{{ trans('loan.admin_fee') }}</label>
                    <div class="col-sm-6">
                        <input type="text" readonly value="{{round($loan->schedule[0]->fee,2)}}" class="form-control" name="admin_fee"/>
                    </div>
                </div>
                <div class="form-group">
                                ...
                                <div id="printArea" class="printArea">
                                    ...
                                    <div class="modal-header bo-border">
                                        <ul id="language" class="pull-right">
                                            @if(ROUND_NUM=='UP')
                                                <li><a class="btn btn-default" id="click_round" href="#"><i class="glyphicon glyphicon-arrow-up"></i> {{trans('multiple.round')}}
                                                    </a></li>
                                            ...
                                            @endif
                                            <li>
                                                <button class="btn btn-warning" id="printer"><i class="fa fa-print"></i> {{ trans('multiple.m_print') }}
                                                </button>
                                            </li>
                                            <li>
                                                <button class="btn btn-warning" id="customer_printer"><i class="fa fa-print"></i> {{trans('loan.l_customer_print')}}
                                                </button>
                                            </li>
                                            <li>
            ...
            <div class="hide">
                <input type="hidden"  id="unit_code" value="{{$loan->units->code}}"/>
                ...
                <input type="hidden" name="balloon_amount_array" id="balloon_amount_array" value="{{$loan->balloon_amount_array}}"/>
                <?php
                ...
                <?php foreach ($repayments as $rep) { ?>
                    <input type="hidden" name="principal_input[]" value="<?php echo $rep->principal ?>" class="principal_input"/>
                <?php } ?>
                <input type="hidden" name="account_name" id="client-name" value="{{$loan->client_loan_account->account_name}}"/>
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        var unit_code = $('#unit_code').val();
            //  $.ajax({
            //     url: "https://contract.chaktomukcity.com/api/unit_actions/contract",
            //     type: 'GET',
            //     headers: {
