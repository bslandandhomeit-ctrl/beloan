...
@section('content')
    <section class="panel">
        ...
        <div class="panel-body">
            ...
            @endif
            <form class="cmxform form-horizontal" method="post" action="{{ route('loan_add_cost',[$loan->id]) }}" id="addCostForm">
                <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                <div class="form-group">
                    <label class="col-sm-3 control-label">{{ trans('loan.l_cost_type') }} <span class="red-color">*</span> </label>
                    <div class="col-sm-8">
                         <select id="cost_type" name="cost_type" class="form-control">
                            <option value="">-</option>
                            <?php
                                $static = Config::get('static_data');
                                if(!empty($static) && !empty($static['fee_cost_type'])){
                                    $loan_cost = $static['fee_cost_type'];
                                    foreach($loan_cost as $key=>$cost){
                                        echo '<option value="'.$key.'">'.$cost.'</option>';
                                    }
                                }
                            ?>
                        </select>
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label">{{ trans('loan.l_cost_date') }} <span class="red-color">*</span></label>
                    <div class="input-append date dpYears col-sm-8" data-date-viewmode="years" data-initialize="datepicker" data-date-format="dd/mm/yyyy" >
                        <input type="text" value=""
                               class="form-control" name="cost_date" id="cost_date">
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label">{{ trans('report.rpt_amount') }} <span class="red-color">*</span></label>
                    <div class="col-sm-8">
                        <input type="text" name="cost_amount" id="cost_amount" class="form-control" required/>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    var parent_credit_label_first = $('.parent_credit_label_className:first').val();
    $('select[name="payment_type"]').change(function () {
        this_ = $(this);
        var coa_dd = <?php echo json_encode($coa_2); ?>;
        var branch_code = "<?php echo $branch_code . '-' ?>";
        var currency = "<?php echo $currency?>"
        if(this_.val() == 0){
             $('.parent_credit_label_className').each(function(){
                 $(this).val(parent_credit_label_first);
             });
            ...
            $.ajax({
                ...
                success: function (res) {
                    res = JSON.parse(res);
                    console.log(res);
                     var credit_value = res.name + " ("+ branch_code + res.account_code + ")";
                    $('.parent_credit_label_className').each(function(){
                        $(this).val(credit_value);
                    });
                    $('.parent_credit_className').each(function(){
                        $(this).val(res.id);
                    });
