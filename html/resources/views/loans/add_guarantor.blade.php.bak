...
@section('content')
    <section class="panel">
        <header class="panel-heading">
           {{ trans('loan.l_add_new_collateral') }}
        </header>
        ...
        <div class="panel-body">
             ...
             @endif
            <form class="cmxform form-horizontal" method="post" action="{{ route('add_collateral', [$loan_id]) }}" id="addCollateralForm" enctype="multipart/form-data">
                        ...
                        <div class="col-sm-6">
                            <select name="collateral_type" id="collateral_type" class="form-control" required>
                               ...
                               <?php $static = Config::get('static_data'); ?>
                                    @if(!empty($static) && !empty($static['collateral_type']))
                                        <?PHP $collateral_type = $static['collateral_type']; ?>
                                        @foreach($collateral_type as $key=>$value)
                                                <option value="{{$key}}"
                                                    <?PHP if(trim(strtolower($collateral->collateral_type)) ==  trim(strtolower($key))){ echo "selected";}
                                                    else if(count($collateral)>0){echo "enabled";} ?>
                                                >{{$value}} ({{$key}})</option>
                                        @endforeach
                                    @endif
                            </select>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        <input type="text" name="collateral_no" id="collateral_no" class="form-control" required/>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-3 control-label">{{ trans('loan.l_guarantor_collateral_value') }} <span class="red-color">*</span></label>
                    <div class="col-sm-6">
                        <input type="text" name="collateral_value" id="collateral_value" class="form-control" required/>
                    </div>
                </div>
                 <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        <select name="collateral_registration" id="collateral_registration" class="form-control" required>
                           ...
                           <?php
                                ...
                                if(!empty($static) && !empty($static['collateral_regis_type'])){
                                    $regis_type = $static['collateral_regis_type'];
                                    foreach($regis_type as $value){
               ...
               <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        <textarea name="collateral_address" id="collateral_address" class="form-control"></textarea>
                    </div>
                </div>
                <div class="form-group">
