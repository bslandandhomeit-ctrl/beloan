...
@section('content')
                        ...
                        <nav class="navbar navbar-default" role="navigation">
                            ...
                            <div class="collapse navbar-collapse navbar-ex1-collapse">
                                <ul class="nav navbar-nav navbar-right">
                                    <li>
                                        <a href="{{ route('get_reschedule_to_approve',[$loan->id]) }}">
                                            <i class="fa fa-adn"></i>
                                            {{ trans('loan.l_approve')  }}
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('get_reschedule_to_reject',[$loan->id]) }}">
                                            <i class="fa fa-ban"></i>
                                            {{ trans('loan.l_reject')  }}
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('view_loan_add_document',[$loan->id]) }}">
                                            <i class="fa fa-eye"></i>
                                            {{ trans('loan.l_view_loan_add_document') }}
                                        </a>
                                    </li>
                                </ul>
                            </div><!-- /.navbar-collapse -->
                        ...
                        <div class="col-md-4">
                            <table class="table table-bordered table-striped table-condensed">
                                ...
                                <tr>
                                    ...
                                    <td>{{ !empty($restructure->loan_installment_duration)?$restructure->loan_installment_duration:"-" }} M</td>
                                </tr>
                                <tr>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label for="m-interest">{{ trans('report.rpt_interest') }} <span
                                                style="color:red">*</span></label>
                                    <input type="text" class="form-control" id="m-interest" name="m-interest"/>
                                </div>
                                <div class="form-group">
                                    <label for="m-principal">{{ trans('report.rpt_principal') }} <span
                                                style="color:red">*</span></label>
    ...
    if (!empty($loan)) {
        ...
        $repay = [
            ...
            'loan_amount' => $restructure->loan_amount,
            'interest_rate' => $restructure->loan_interest_rate,
            'loan_duration' => $restructure->loan_loan_duration,
            'repayment_type' => $restructure->loan_repayment_type,
            'num_balloon' => $restructure->loan_balloon,
            'balloon_month' => $restructure->loan_balloon_month,
            'balloon_amount' => $restructure->loan_balloon_amount_array,
            'monthly_payment' => $restructure->loan_monthly_payment,
            'loan_status' => $loan->status,
            'disbursement_date' => $loan->disburse_date,
            'admin_fee' => $restructure->loan_admin_fee,
            'maintain_fee' => $restructure->loan_maintain_fee,
            'admin_fee_opt' => $restructure->loan_admin_fee_opt,
            'maintain_fee_opt' => $restructure->loan_maintain_fee_opt,
            'other_fee' => $restructure->loan_other_fee
        ];
    }
    ?>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="col-md-12">
                            <div data-initialize="datepicker" data-date-format="yyyy/mm/dd"
                                 data-date="{{date('Y-m-d')}}" class="input-append date retrieve_date">
    ...
    <!-- Reject -->
    <div aria-hidden="true" role="dialog" id="reject_modal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button aria-hidden="true" data-dismiss="modal" class="close" type="button">×</button>
                ...
                <div class="modal-body">
                    <div class="col-md-12">
                        <textarea class="form-control" id="description" name="description" placeholder="{{ trans('multiple.m_description') }}" cols="10" rows="10"></textarea>
                        <span id="msg" class="red"></span>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="modal-footer">
                    <input type="button" id="submit_to_reject" name="submit_frm" class="btn btn-primary" value="{{ trans('multiple.m_save') }}"/>
                </div>
            </div>
        </div>
    </div>
...
@section('js')
    ...
    <script type="text/javascript" src="{{ asset('js/reschedule-detail.js',isset($secure) ? false : false) }}"></script>
    <script type="text/javascript"
            src="{{ asset('theme/js/iCheck/jquery.icheck.js',isset($secure) ? false : false)}}"></script>
    <script type="text/javascript"
            src="{{ asset('theme/js/bootstrap-datepicker/js/bootstrap-datepicker.js',isset($secure) ? false : false)}}"></script>
    ...
    <script type="text/javascript">
        ...
        $('body').on('click','#approve',function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.status == true){
                        window.location.href = '{{ route('waiting_to_approve') }}';
                    }
                }
            });            
        });

        $('body').on('click','#submit_to_reject',function(){
