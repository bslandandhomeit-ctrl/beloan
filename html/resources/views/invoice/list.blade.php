...
<section class="panel">
    <header class="panel-heading">
        <span>Invoice Order List</span>       
    </header>
    <div class="panel-body">
        <div>
            <form role="form" class="cmxform form-horizontal" id="search_frm" method="get" action="{{ route('list_invoice') }}">
            <div class="row">
                    <div class="col-md-3">                                       
                    ...
                    <tbody>
                        @forelse($lists as $d)
                            <tr>
                                ...
                                <td class="isVerticalalign">{{ $d->invoice_no}}</td>
                                ...
                                <td class="isVerticalalign">{{ $d->discount_payment_option }}</td>
                                <td class="isVerticalalign">${{ number_format($d->price_after_discount,2) }}</td>
                                <td class="isVerticalalign">{{ $d->diposit_amount }}</td>
                                <td class="isVerticalalign">${{ number_format($d->final_price,2) }}</td>                          
                                <td class="isVerticalalign">
                                    @if($d->invoice_status =='Invoice')
                                        <span class="btn btn-default" title="{{$d->invoice_status}}">{{$d->invoice_status}}</span>                                        
                                    @elseif($d->invoice_status =='Void')
                                        <span class="btn btn btn-danger" title="{{$d->invoice_status}}">{{$d->invoice_status}}</span>
                                    @elseif($d->invoice_status =='Canceled')
                                    <span class="btn btn-danger" title="{{$d->invoice_status}}">{{$d->invoice_status}}</span>
                                    @else
                                    <span class="btn btn-dark" title="{{$d->invoice_status}}">{{$d->invoice_status}}</span>                                    
                                    @endif  
                                </td> 
                                <td class="isVerticalalign">{{ $d->remark }}</td> 
                                
                                <td class="isVerticalalign define-width" align="center">
                                    <a href="{{ route('invoice_detail',[$d->id]) }}" class="btn btn-xs btn-default" title="Detail"><i class="fa fa-search-minus"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
