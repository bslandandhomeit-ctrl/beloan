        ...
        <div class="col-sm-12">
            <section class="panel">
                <header class="panel-heading">
                    {{ trans('promotion.discount_promotion') }} : {{ $promotion->name }}
                </header>
                <div class="panel-body">
                    ...
                    <form class="cmxform form-horizontal" method="post" action="{{route('edit_promotion',['id' => $promotion->id])}}" id="frm_promotion">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label class="control-label">{{ trans('promotion.name') }}:<span class="red-color"> *</span></label>
                                    <input type="text" value="{{ $promotion->name }}" id="name" name="name" class="form-control" required/>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <div data-date-viewmode="years" data-initialize="datepicker" data-date-format="yyyy-mm-dd" class="input-append date start_date">
                                        <input type="text" value="{{ $promotion->start_date }}" id="start_date"  name="start_date" size="16" class="form-control" >
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <div data-date-viewmode="years" data-initialize="datepicker" data-date-format="yyyy-mm-dd" class="input-append date end_date">
                                        <input type="text" value="{{ $promotion->end_date }}" id="end_date" name="end_date" size="16" class="form-control" >
                                        <span class="add-on date">
                                            <button class="btn btn-primary" type="button"><i class="fa fa-calendar"></i></button>
                                        </span>
                                    </div>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <div class="input-group" style="margin-top: 5px;">
                                        ...
                                        <input type="text" value="{{ $promotion->discount_amount }}" id="discount_amount" name="discount_amount" class="form-control"/>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('js')
    ...
    <script>
        ...
        old_data = <?php echo $unit_type_promotion;?>;
        if(old_data != ''){
            for(var i = 0; i < old_data.length; i++) {
                data_arr[old_data[i].id] = old_data[i];
            }
        }
        $(document).ready(function () {
            ...
            $('body').on('click','.btn_remove',function(){
                ...
                if(confirm('Do you want to delete?') == true){
                    if(id != ''){
                        $.ajax({
                            url: '{{ route('remove_unit_type_promotion') }}',
                            type: 'GET',
                            dataType: 'json',
                            data: {id: id},
                    ...
                    html += `
                        <div class="row rows" id="row${data.id}" style="padding: 10px 0px 10px;border-bottom: 1px solid #eee;">
                            ...
                            <div class="col-sm-2 text-right">
                                <button type="button" class="btn btn-danger btn_remove" data-id="${data.unit_type_promotion_id}" data-index="${data.id}" style="background-color: #d9534f !important;">{{ trans('multiple.m_remove') }}</button>
                            </div>
                       </div>
                    `;
                });
