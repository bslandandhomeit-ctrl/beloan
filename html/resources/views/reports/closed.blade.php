...
@section('content')
    <section class="panel">
        <header class="panel-heading">
            {{ trans('report.rpt_closed_loan_report') }}
            @if(isset($start) && isset($end))
                {{ trans('multiple.m_from') }} {{ date("d-M-Y", strtotime($start)) }} {{ trans('multiple.m_to') }} {{ date("d-M-Y", strtotime($end)) }}
            @else
                {{ isset($start)?'Loan closed on'.date("d-M-Y", strtotime($start)):'' }}
                {{ isset($end)?'Loan closed on '.date("d-M-Y", strtotime($end)):'' }}
            @endif
            @foreach($branch as $b)
                @if(isset($branch_id))
                    @if($branch_id==$b->id)
        ...
        <div class="panel-body">
            <div class="position-center" style="width:100%;">
                <form role="form" method="get" action="{{ route('rpt_closed') }}">
                    <div class="row">
            ...
            <div id="printArea" style="clear: both">
                ...
                <section id="unseen">
                    <table class="table table-bordered table-striped table-condensed closed" id="closed">
                                ...
                                <tr>
                                    ...
                                    <td align="center">{{ Date("d-M-Y", strtotime($set->settlement_date)) }}</td>
                                        <?php
                                            ...
                                            echo '<td align="right">'.$new_date.'</td>';
                    ...
                    <div>
                        @include('partials.pagination',['results'=>$closed])
                    </div>
                </section>
            </div>
        </div>
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        $("#export").click(function (event) {
            ...
            if(con == true){
                new TableExport(document.getElementById('closed'), {
                    formats: ['csv'],
                    filename:"closed"
                });
