...
<div id="divTab">
    <table class="table table-striped table-hover table-bordered irregular" id="listtable">
        <thead class="th-center" style="background-color: #ffffff">
        <tr class="header">
            ...
            <th rowspan="2" style="vertical-align: middle;">
                {{ trans('report.rpt_maturity_date') }}
            </th>
        ...
        <tr class="head-1">
            ...
            <th style="vertical-align: middle;">
                {{ trans('report.rpt_loan_amount') }}
            </th>
            <th style="vertical-align: middle;">
                {{ trans('report.rpt_int_rate') }}
            </th>
            <th>
               {{ trans('report.rpt_tenure') }}
            </th>
        ...
        <tbody>
            ...
            <tr>
                ...
                if (!empty($payment) && count($payment) > 0) {
                    $last_payment = $payment->where('status', 0, false)->first();
                ...
                $maturity_date = !empty($l->maturity_date) ? $l->maturity_date : $repayment_array_global[count($repayment_array_global) - 1][0];
                // Balance
                $prin_balance = $l->loan_amount - $total_prin;
                $sumBalance += $prin_balance;

                $total_row[0] += $to_interest;
                $total_row[1] += $to_principal;
                $total_row[2] += $to_penalty;
                $total_row[3] += $total_payment;
                ?>
                <td>{{$n}}</td>
		...
		<tfoot>
			<tr style="font-weight: bold;">
                ...
                <td align="right">{{number_format($total_row[0],2,'.',',')}}</td>
                <td align="right">{{number_format($total_row[1],2,'.',',')}}</td>
                <td align="right">{{number_format($total_row[2],2,'.',',')}}</td>
                <td align="right">{{number_format($total_row[3],2,'.',',')}}</td>
                <td colspan="5"></td>
            </tr>
		</tfoot>
    </table>
