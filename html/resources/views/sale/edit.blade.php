...
@section('content')
    <section class = "panel">
        ...
        <div class = "panel-body">
            ...
            <form class = "cmxform form-horizontal" method = "post" action = "{{route('edit_sale')}}" id = "frmSale">
                <div class = "col-lg-12">
                        ...
                        <select  class = "select2_type customer_select1"required style = "width: 100%;" name = "branch" id = "branch">
                                @if(isset($branches))
                                    ...
                                    @foreach($branches as $b)
                                        <option value = "{{ $b->branch_code }}" {{ ($saleItem->company_branch_id == $b->id)?'selected' : "" }} company-id="{{ $b->id }}">{{ $b->branch_name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class = "form-group">
                        ...
                        <label class="control-label">Discount (Promotion)<span class="red"> *</span></label>
                            <input type="text" class="form-control" id="discount_promotion" name="discount_promotion" readonly value="{{$sale->discount_promotion}}"/>
                        </div>
                    </div>
                    <div class = "form-group">
                        <div class="col-md-6 hiden">
                            <label class="control-label">Discount (Other)<span class="red"> *</span></label>   
                            <input type="text" class="form-control" id="discount_other" name="discount_other" value="{{$sale->discount_other}}"/>
                        </div>
                        <div class="col-sm-6 hiden">
                        <label class="control-label">{{ trans('loan.clearance_amount')}}</label>
                            <input type="text" class="form-control" id="clearance_amount" name="clearance_amount" value="{{$sale->clearance_amount}}"/>
                        </div>
                    </div>
                    <div class = "form-group hiden">   
                        <div class="col-md-6">
                        ...
                        <label class="control-label" style="width: 100%;text-align: left;">Status<span class="red"> *</span></label> 
                            <div class="form-check form-check-inline">                                    
                                    <input  class="form-check-input" type="radio" id="new_sale" name="sale_status" value="New_Sale" {{$sale->sale_status==='New_Sale'?"checked":''}} >
                                    <label class="form-check-label" for="inlineRadio1">New Sale</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                    
                                    <input  class="form-check-input" type="radio" id="change_unit" name="sale_status" value="Change_Unit" {{$sale->sale_status==='Change_Unit'?"checked":''}}>
                        ...
                        <div class="col-md-12">
                            ...
                            <option value="">-</option>
                                @foreach($saleRepresentative as $c)
                                    <option value={{$c['id']}} @if($sale->sale_person==$c['id']) selected="selected" @endif>{{$c['name']}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>    
                    ...
                    <div class = "form-group">
                        <div class="col-md-12">
                            ...
                            <textarea name="remark" id="remark" class="form-control" rows="2">{{$sale->remark}}</textarea>
                        </div>
                    </div>
                    <div class = "form-group">
                    ...
                    <div class = "form-group">
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control" id="diposit_amount" name="diposit_amount" value="{{$sale->diposit_amount}}"/>
                        </div>
                    </div>
                    <div class = "form-group">
                        <div class="col-sm-6 text-right">
...
<script type="text/javascript" src="{{ asset('theme/js/jquery.validate.min.js',false) }}"></script>
    ...
    <script type = "text/javascript">
        ...
        $(document).ready(function () {
            ...
            var customer_id = '<?php echo $sale->client_id;?>';
            $("#customer_id").select2({
        ...
        var company = '<?php echo $saleItem->company_branch_id;?>';
        if(company){
            getProjectByCompany();
        ...
        $(document).on('change', '#unit_id', function () {
            getUnitInfo();
            var payment_option = '<?php echo $sale->payment_option;?>';
                    if(payment_option){
                        $("#payment_option").val(payment_option).trigger('change');
                    }
        });
        ...
        function getProjectByCompany(){
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#project_id").select2();
                    var project_id = '<?php echo $saleItem->project_id;?>';
                    if(project_id){
        ...
        function getUnitType(){
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#unit_type_id").select2();
                    var unit_type_id = '<?php echo $saleItem->unit_type_id;?>';
                    if(unit_type_id){
                        $("#unit_type_id").val(unit_type_id).trigger('change');
                        $.ajax({
                        url: '/sale/get-payment-term',
                        type: 'GET',
                        dataType: "json",
                        success: function (data) {                   
                                $('#payment_option_type').html(data.option);
                                $("#payment_option_type").select2();
                                var payment_option_type = '<?php echo $sale->payment_option_type;?>';
                                if(payment_option_type){
                                    $("#payment_option_type").val(payment_option_type).trigger('change');
                                }else{
        ...
        function getUnitByUnittype(){
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#unit_id").select2();
                    var unit_id = '<?php echo $saleItem->unit_id;?>';
                    if(unit_id){
