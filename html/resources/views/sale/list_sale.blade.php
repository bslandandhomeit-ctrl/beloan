...
<section class="panel">
    ...
    <div class="panel-body">
        <div>
            <form role="form" class="cmxform form-horizontal" id="search_frm" method="get" action="{{ route('list_sale') }}">
            <div class="row">
                    <div class="col-md-3">                                       
        ...
        <div id="printArea" class="ox-scroll">
            ...
            <section id="unseen" >
                <table class="table table-bordered table-striped table-condensed table-hover dealerTable" id="dealers_list">
                    ...
                    <th style="vertical-align: middle;text-align: center;width:200px;">Issue Date</th>
                        ...
                        <?php $i++;?>
                            <tr>
                                ...
                                <td class="isVerticalalign">                              
                                    @if($d->status =='Ordered')
                                        <?php if($d->sale_status =='Change_Unit'){?>
                                        <span class="btn btn-warning" title="{{$d->status}}">{{$d->diposit_amount>0?'Deposit':'Change Unit'}}</span>
                                        <?php }else{?>
                                        <span class="btn btn-success" title="{{$d->status}}">{{$d->diposit_amount>0?'Deposit':'New Sale'}}</span>
                                        <?php }?>  
                                    @elseif($d->status =='Accepted' && $d->invoice_status!='Invoice')
                                    <span class="btn btn-primary" title="{{$d->status}}"> Created Drawdown</span>
                                    @elseif($d->status =='Accepted' && $d->invoice_status=='Invoice')
                                    <span class="btn btn-default" title="{{$d->status}}"> Disbursed</span>
                                ...
                                <!-- <td class="isVerticalalign">
                                    @if($d->invoice_status =='First Created')
                                        <span class="btn btn-success" title="{{$d->invoice_status}}">{{$d->invoice_status}}</span>
                                    @elseif($d->invoice_status =='Modified')
                                        <span class="btn btn-warning" title="{{$d->invoice_status}}">{{$d->invoice_status}}</span>
                                    @else
                                    <span class="btn btn-default" title="{{$d->invoice_status}}">{{$d->invoice_status}}</span>                                    
                                    @endif  
                                </td>  -->
                                ...
                                <td class="isVerticalalign define-width" align="center">
                                    ...
                                    @endif
                                        @if($d->status =='Accepted')
                                            <a href="{{route('transferBalance', [$d->unit_id])}}" class="btn btn-xs btn-default" title="Transfer"><i class="fa fa-exchange" aria-hidden="true"></i></a>
                                        @endif
                                </td>
                            </tr>
                        @endforeach
