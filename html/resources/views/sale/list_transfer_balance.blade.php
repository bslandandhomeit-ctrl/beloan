...
<section class="panel">
    <header class="panel-heading">
        <span>Transfer balance list</span>
...
@endif
            ...
            <form role="form" class="cmxform form-horizontal" id="search_frm" method="get" action="{{ route('getTransferBalanceList') }}">
            <div class="row">
                    <div class="col-md-3">                                            
        ...
        <div id="printArea" class="ox-scroll">
            ...
            <input type = "hidden" name = "sale_person" value = "{{ Request::get('sale_person') }}"/>
                ...
                <table class="table table-bordered table-striped table-condensed table-hover dealerTable" id="dealers_list">
                    ...
                    <th style="vertical-align:middle; text-align: center;">From Accounte</th>   
                    <th style="vertical-align:middle; text-align: center;">Account Number</th>              
                    <th style="vertical-align:middle; text-align: center;">Total principal paid</th>
                    <th style="vertical-align:middle; text-align: center;">Total Interest paid</th>   
                    <th style="vertical-align:middle; text-align: center;">Total Payment</th> 
                    <th style="vertical-align:middle; text-align: center;"></th> 
                    <th style="vertical-align:middle; text-align: center;">To Account</th>                  
                    <th style="vertical-align:middle; text-align: center;">Account Number</th>               
                    ...
                    <tbody class="checkbox-group">
                        ...
                        @forelse($lists as $d)
                        <?php $i++;
                        $total_payment=floatval($d->total_principal_paid) + floatval($d->total_interest_paid);
                        ?>
                            <tr>
                            <!-- <td class="isVerticalalign" align="center"><input type="checkbox" id="{{ $i}}" name="comission_request[]" value="{{$d->id}}" /></td>  -->
                                ...
                                <td class="isVerticalalign">{{ !empty($d->created_at)?date('d-M-Y',strtotime($d->created_at)):"N/A" }}</td>                      
                                <td class="isVerticalalign">{{ $d->from_account_name }}</td> 
                                <td class="isVerticalalign">{{ $d->from_account_no }}</td>   
                                <td class="isVerticalalign">{{  number_format($d->total_principal_paid,2,'.',',') }}</td> 
                                <td class="isVerticalalign">{{  number_format($d->total_interest_paid,2,'.',',') }}</td> 

                                <td class="isVerticalalign">{{  number_format($total_payment,2,'.',',') }}</td> 
                                <td class="isVerticalalign"><i class="fa fa-exchange" aria-hidden="true"></i></td> 

                                <td class="isVerticalalign">{{ $d->to_account_name }}</td> 
                                <td class="isVerticalalign">{{ $d->to_account_no }}</td>   
                                ...
                                <td class="isVerticalalign define-width" align="center">                                                                     
                                    ...
                                    {{$d->status}}
                                    @else
                                        <a href="{{ route('transfer_balance_approve', [$d->id])}}" class="btn btn-xs btn btn-primary" title="Edit"><i class="fa fa-pencil"></i> Approved By</a>
                                        <a href="{{ route('transfer_balance_reject', [$d->id])}}" class="btn btn-xs btn btn-danger" title="Reject"><i class="fa fa-times"></i> Reject</a> 
                                    @endif
                                </td>         
                            </tr>
                        @endforeach
